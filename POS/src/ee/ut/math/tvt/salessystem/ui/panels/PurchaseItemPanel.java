...
public class PurchaseItemPanel extends JPanel {
	...
	private JComponent drawDialogPane() {
		...
		while (i <= model.getWarehouseTableModel().getRowCount()) {
			...
			String esemeNimi;
			long esemeBarCode;
			Asjad.add(model.getWarehouseTableModel().getItemById(i));
			//System.out.println(model.getWarehouseTableModel().getItemById(i));
			int j = i - 1;
			esemeNimi = Asjad.get(j).getName();
			esemeBarCode = Asjad.get(j).getId();
			productName.addItem(esemeNimi);
			barcode.add(esemeBarCode);
			i++;
		}
		...
		// - ScrollMenu where the items are
	...
	public void itemSelectedEventHandler() {
		// which item is selected
		int indeks = productName.getSelectedIndex();
		indeks = indeks + 1;
		String indeks_string = Integer.toString(indeks);
		barCodeField.setText(indeks_string);
		StockItem HetkeElement = model.getWarehouseTableModel().getItemById(
				indeks);
		double hind = HetkeElement.getPrice();
		String hind_string = Double.toString(hind);
		priceField.setText(hind_string);
	}
	...
	public void setEnabled(boolean enabled) {
		this.productName.setEnabled(enabled);
